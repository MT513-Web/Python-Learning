# 🐍 Python Learning Journey

Welcome to my Python practice repository! I am a **BSCS Student**, and this is where I document my daily progress, assignments, and mini-projects.

## 🚀 Daily Progress Tracker

| Day | Topic | Key Concepts |
| :--- | :--- | :--- |
| **Day 1-2** | Basics | Variables, Data Types, and Operators |
| **Day 3** | Conditionals | `if-elif-else` Logic & Grade Checker Project |
| **Day 4** | Loops | `for` & `while` Loops, Table Generator |
| **Day 5** | Strings | Indexing, Slicing, and String Methods |
| **Day 6** | Lists | `.append()`, `.remove()`, `.sort()`, and Loops |
| **Day 7** | Dictionaries | `Key:Value` Pairs, Dynamic Input, & Data Retrieval |
| **Day 8** | **Mini Project** | **Smart Phonebook** (Validation, Search & Delete) |

## 🛠️ Projects Included
* **Grade Checker:** A system to calculate grades based on user input.
* **Table Generator:** A tool that generates multiplication tables for any number.
* **Smart Phonebook:** A contact management app with search and delete features.

---
*Follow my journey on [LinkedIn](https://www.linkedin.com/in/ayesha-afzal-)*
